 ...
 */
    ...
    const Cleanup = {
        ...
        render() {
            UI.el('tpm-pane-cleanup').innerHTML = `
              <div class="tpm-section" id="tpm-slow-resume" style="display:none">
                ...
                <p class="tpm-warn-box" style="margin:0 0 10px"><strong id="tpm-slow-resume-n">0</strong> deleted so far from the <strong id="tpm-slow-resume-tab">Replies</strong> tab. A blank or stuck timeline reloads itself and continues. Stop only if you want to end the session. <strong id="tpm-slow-resume-crash"></strong></p>
                <div class="tpm-btns">
              ...
              <div class="tpm-section">
                ...
                <p style="margin-top:10px">Two separate runs, no export. <strong>Posts</strong> is original tweets only. <strong>Replies</strong> is replies only. Run each when you want that tab cleaned. Pause and Stop apply to the active run and to file-based delete.</p>
              </div>
            ...
            if (session && session.active) {
                ...
                if (session.autoResume && !session.userPaused) {
                    ...
                    if (onProfile) {
                        ...
                        location.replace(this.profileTabUrl(session.tab || 'replies'));
                    }
        ...
        profileTabUrl(tab) {
            const u = Core.username;
            if (!u) return Core.baseUrl;
            return tab === 'replies'
                ? `${Core.baseUrl}/${u}/with_replies`
                : `${Core.baseUrl}/${u}`;
        },

        clickProfileTab(tab) {
            const user = (Core.username || '').toLowerCase();
            const links = Array.from(document.querySelectorAll('[data-testid="ScrollSnap-List"] a'));
            const hit = links.find((a) => {
                const href = (a.getAttribute('href') || '').toLowerCase();
                const text = (a.textContent || '').trim().toLowerCase();
                if (tab === 'replies') return href.includes('/with_replies') || text === 'replies';
                return !href.includes('/with_replies') && (text === 'posts' || text === 'tweets' || (user && (href === `/${user}` || href === `/${user}/`)));
            });
            if (hit) hit.click();
            return !!hit;
        },

        onWantedProfileTab(tab) {
            const user = (Core.username || '').toLowerCase();
            if (!user) return false;
            const path = location.pathname.toLowerCase();
            const onProfile = path === `/${user}` || path === `/${user}/` || path.startsWith(`/${user}/`);
            if (!onProfile) return false;
            const onReplies = path.includes('/with_replies');
            return tab === 'replies' ? onReplies : !onReplies;
        },
        ...
        // stripped tweet innards, so the next pass found no carets and jumped.
        hideTimelineSuggestions() {
            document.querySelectorAll('section [data-testid="cellInnerDiv"]').forEach(cell => {
                if (cell.querySelector('[data-testid="tweet"]')) return;
                if (cell.querySelector('[data-testid="UserCell"]')) cell.remove();
            });
        },
        ...
        async slowDelete(resume = false, tab) {
            ...
            if (resume) {
                ...
                if (!window.confirm(`Slow delete will permanently delete items on your ${tabName} tab. This cannot be undone. Continue?`)) return;
                session = {
                    active: true, deleted: 0, tab,
            ...
            this.info(`Slow deleting the ${tab === 'posts' ? 'Posts' : 'Replies'} tab…`);

            if (!this.onWantedProfileTab(tab)) {
                session.autoResume = 1;
                Core.store.set(this.slowSessionKey, session);
                this.info(`Opening your ${tab === 'posts' ? 'Posts' : 'Replies'} tab…`);
                await Core.sleep(800);
                location.replace(this.profileTabUrl(tab));
                return;
            }
            this.clickProfileTab(tab);
            ...
            while (true) {
                ...
                try {
                    ...
                    if (moreElement) moreElement.scrollIntoView({ block: 'center', behavior: 'instant' });
