 ...
 */
    ...
    const Cleanup = {
        ...
        render() {
            UI.el('tpm-pane-cleanup').innerHTML = `
              <div class="tpm-section" id="tpm-slow-resume" style="display:none">
                ...
                <p class="tpm-warn-box" style="margin:0 0 10px"><strong id="tpm-slow-resume-n">0</strong> deleted so far from the <strong id="tpm-slow-resume-tab">Replies</strong> list. Resume continues the GraphQL run (no scrolling). <strong id="tpm-slow-resume-crash"></strong></p>
                <div class="tpm-btns">
              ...
              <div class="tpm-section">
                ...
                <div class="tpm-btns">
                  <button id="tpm-slow-posts" class="tpm-btn tpm-btn-ghost" type="button">Slow delete Posts</button>
                  <button id="tpm-slow-replies" class="tpm-btn tpm-btn-ghost" type="button">Slow delete Replies</button>
                </div>
                ...
                <p style="margin-top:10px">Two separate GraphQL runs, no export and no timeline scrolling. <strong>Posts</strong> deletes original tweets; <strong>Replies</strong> deletes replies. Pause and Stop apply to the active run and to file-based delete.</p>
              </div>
            ...
            UI.el('tpm-slow-posts').onclick = () => { if (!this.running) this.slowDelete(false, 'posts'); };
            UI.el('tpm-slow-replies').onclick = () => { if (!this.running) this.slowDelete(false, 'replies'); };
            ...
            if (session && session.active) {
                ...
                const tabEl = UI.el('tpm-slow-resume-tab');
                if (tabEl) tabEl.textContent = (session.tab === 'posts') ? 'Posts' : 'Replies';
                ...
                if (session.autoResume && !session.userPaused) {
                    this.info('Resuming GraphQL slow delete…');
        ...
        setLiveButtons() {
            ...
            ['tpm-slow-posts', 'tpm-slow-replies'].forEach((id) => {
                const el = UI.el(id);
                if (el) el.disabled = this.running;
            });
        },
        ...
        persistSlowFlags() {
            ...
            if (this._slowTab) session.tab = this._slowTab;
            session.tIds = this.tIds || [];
            session.deleted = this.dCount;
            session.total = this.total;
            session.sparedCount = this.sparedCount;
            session.beat = Date.now();
            session.autoResume = this.paused ? 0 : 1;
        ...
        async slowDelete(resume = false, tab) {
            let session = Core.store.get(this.slowSessionKey, null);
            if (resume) {
                if (!session || !session.active) return;
                tab = session.tab === 'posts' ? 'posts' : 'replies';
            } else {
                tab = tab === 'posts' ? 'posts' : 'replies';
                const tabName = tab === 'posts' ? 'Posts' : 'Replies';
                if (!window.confirm(`Slow delete will permanently delete your ${tabName} via GraphQL. This cannot be undone. Continue?`)) return;
                session = {
                    active: true, deleted: 0, tab, tIds: [],
            ...
            this.running = true;
            this.stopFlag = false;
            this.paused = !!session.userPaused;
            this.setLiveButtons();
            this.dCount = session.deleted || 0;
            this.sparedCount = session.sparedCount || 0;
            this.createProgressBar();

            try {
                if (resume && Array.isArray(session.tIds) && session.tIds.length) {
                    this.tIds = session.tIds.slice();
                    this.total = (session.total || (this.dCount + this.tIds.length));
                    this.info(`Resuming ${this.tIds.length} remaining ${tab}…`);
                } else {
                    this.info(`Listing live ${tab} via GraphQL…`);
                    const skipDays = parseInt(UI.el('tpm-skipDays')?.value, 10) || 0;
                    const all = await SlowGql.collect(tab, (page, neu, total) => {
                        this.info(`Listing ${tab}: page ${page}, ${total} found…`);
                    });
                    if (this.stopFlag) {
                        this.info(`Stopped while listing. ${this.dCount} deleted.`);
                        return;
                    }
                    const keep = [];
                    let spared = 0;
                    all.forEach((t) => {
                        if (SlowGql.eligible(t, tab, skipDays, this.spareThreshold)) keep.push(t.id);
                        else spared++;
                    });
                    this.tIds = keep;
                    this.sparedCount += spared;
                    this.total = this.dCount + keep.length;
                    this.info(`Listed ${all.length}. Deleting ${keep.length} ${tab}, sparing ${spared}.`);
                    this.persistSlowFlags();
                    this.updateProgressBar();
                }
                if (!this.tIds.length) {
                    Core.store.set(this.slowSessionKey, null);
                    this.info(`Nothing to delete on ${tab}. Spared ${this.sparedCount}.`);
                    return;
                }
                await this.deleteTweets();
                if (this.stopFlag) {
                    this.persistSlowFlags();
                    this.info(`Stopped. ${this.dCount} deleted, ${this.tIds.length} remaining.`);
                } else {
                    Core.store.set(this.slowSessionKey, null);
                    this.info(`Finished ${tab}. Deleted ${this.dCount}. Spared ${this.sparedCount}.`);
                }
            } catch (err) {
                console.error('[TPM] GraphQL slow delete failed:', err);
                session = Core.store.get(this.slowSessionKey, null) || session;
                session.active = true;
                session.autoResume = 0;
                session.tIds = this.tIds || [];
                session.deleted = this.dCount;
                Core.store.set(this.slowSessionKey, session);
                this.info(`Slow delete failed: ${(err && err.message) || err}. Press Resume to retry.`);
            } finally {
                this.running = false;
                this.setLiveButtons();
            }
